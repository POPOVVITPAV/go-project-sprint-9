...
func Generator(ctx context.Context, ch chan<- int64, fn func(int64)) {
	// 1. Функция Generator
	fmt.Println("Начало работы Generator")
	...
	for {
		...
		case <-ctx.Done():
			close(ch)
			fmt.Println("Прервали работу Generator")
			return
		default:
			...
			// Вызываем функцию fn
			fn(n)
			// Увеличиваем значение на 1 для следующей итерации
			n++
		}
	}
}
...
func main() {
	...
	for i, in := range outs {
		...
		go func(in <-chan int64, i int) {
			defer wg.Done()
			for v := range in {
				// Увеличиваем счётчик в слайсе amounts
				amounts[i]++
